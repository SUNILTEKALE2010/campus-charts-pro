...
import { parseAttendance } from "./attendance.server";
import type { AttendancePayload } from "./attendance.types";

const SPREADSHEET_ID = "1-eV-RVBO4g-pFMhp0xuos7ge-dAleWpjfo1Q4nbQjd0";
const GATEWAY_URL = "https://connector-gateway.lovable.dev/google_sheets/v4";
const CACHE_TTL_MS = 60_000;

let cache: { data: AttendancePayload; at: number } | null = null;
const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

export const getStudentAttendance = createServerFn({ method: "GET" }).handler(
  async (): Promise<AttendancePayload> => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      res = await fetch(
        `${GATEWAY_URL}/spreadsheets/${SPREADSHEET_ID}/values/Sheet4!A1:Z2000`,
        {
          headers: {
    ...
    const { students, depts, monthLabels } = parseAttendance(data.values ?? []);
