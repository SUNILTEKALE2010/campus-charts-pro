...
import { parseToday } from "./today.server";
...
let cache: { data: TodayPayload; at: number } | null = null;
...
export const getTodayAttendance = createServerFn({ method: "GET" }).handler(
  async (): Promise<TodayPayload> => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      res = await fetch(
        `${GATEWAY_URL}/spreadsheets/${SPREADSHEET_ID}/values/Sheet5!A1:Z2000`,
        {
          headers: {
    ...
    const { students, depts } = parseToday(data.values ?? []);
